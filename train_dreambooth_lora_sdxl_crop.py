...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="/home/dell/workspace/models/stable-diffusion-xl-base-1.0/vae_fp16_fix",
    ...
    parser.add_argument("--crop_size", type=int, default=768, help="for data argument.")
    parser.add_argument(
        "--resolution",
        type=int,
        default=1024,
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_images_path = [i for i in self.instance_images_path if i.suffix in [".png", ".jpeg", ".jpg", ".png", "jpeg", "jpg"]]
        
        self.num_instance_images = len(self.instance_images_path)
        print(f"self.num_instance_images {self.num_instance_images}")
        ...
        self.ori_image_transforms = transforms.Compose(
            [
                # transforms.Resize(size, interpolation=transforms.InterpolationMode.BILINEAR),
                # transforms.CenterCrop(size) if center_crop else transforms.RandomCrop(size),
                transforms.ToTensor(),
    ...
    def __getitem__(self, index):
        ...
        m = max(width, height)
        rat = m/1280
        w_, h_ = int(width / rat), int(height / rat)
        width, height = map(lambda x: max(math.ceil(x / 8) * 8, 512), (w_, h_))
        # w_, h_, rat

        if not instance_image.mode == "RGB":
            instance_image = instance_image.convert("RGB")
        example["instance_images"] = self.image_transforms(instance_image)
        example["ori_images"] = instance_image.resize(
                (width, height),
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    ori_pixel_values = [example["ori_images"] for example in examples]
    ...
    # ori_pixel_values = torch.stack(ori_pixel_values)
    # ori_pixel_values = ori_pixel_values.to(memory_format=torch.contiguous_format).float()
    batch = {"pixel_values": pixel_values, "ori_pixel_values": ori_pixel_values}
    return batch
...
def main(args):
    ...
    def compute_time_ids_argument(original_size, target_size, crops_coords_top_left):
        # Adapted from pipeline.StableDiffusionXLPipeline._get_add_time_ids
        # original_size = (args.resolution, args.resolution)
        # target_size = (args.resolution, args.resolution)
        # crops_coords_top_left = (args.crops_coords_top_left_h, args.crops_coords_top_left_w)
    ...
    print(f">> len data {len(train_dataset)}, {len(train_dataloader)}")
    ...
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    print(f">> debug:args.gradient_accumulation_steps {args.gradient_accumulation_steps}", num_update_steps_per_epoch)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # ori_pixel_values = batch["ori_pixel_values"].to(dtype=vae.dtype)
                ...
                if not args.iscrop or w - crop_sz[0] < 10:
                    ...
                    # crop_sz = (512, 512)
                    # 计算剪裁点的范围
                    x = random.randint(0, w - crop_sz[0])
                    y = random.randint(0, h - crop_sz[1])
                    crop_img = ori_image[0].crop((x, y, x + crop_sz[0], y + crop_sz[1]))
                    crop_pix_values = train_dataset.ori_image_transforms(crop_img)
                    crop_pix_values = torch.stack([crop_pix_values])
    ...
    if accelerator.is_main_process:
        ...
        # xl_model_cfg["base"], torch_dtype=torch.float16, use_safetensors=True, variant="fp16")
        pipeline = StableDiffusionXLPipeline.from_pretrained(
